...
def ImgNormalizer(img,size,padding=2):
    w,h = img.shape
    DrawX = []
    DrawY = []
    for x in range(w):
    ...
    if not DrawX:
        imgSmall = np.full((size,size), 0, dtype=np.uint8)
        imgSmall2 = np.full((size,size), 0, dtype=np.uint8)
    ...
    DrawSmall = []
    array_size = size - 1
    array_size_math = array_size - (2*padding)
    for i in range(len(DrawX)):
        # Reduz a amplitude para 28x28 com 2 pixels de borda e arredonda os resultados
        newpair = []
        newpair.append(((array_size_math/Amp)*DrawX[i]) + padding)
        newpair.append(((array_size_math/Amp)*DrawY[i]) + padding) 
...
class JanelaInicial:
    def __init__(self,path):
        ...
        tk.Button(self.controls_frame, text="Clear", command=self.clear_canvas).grid(row=1,column=0,pady=5)
        ...
        #Escolha de tamanho
        self.Size_selection_frame = tk.Frame(self.controls_frame)
        self.Size_selection_frame.grid(row=3, column=0, pady=5)
        self.size_label = tk.Label(self.Size_selection_frame, text="Choose a size (28 by default):")
        self.size_label.grid(row=0, column=0, pady=5)
        self.size_desenho = tk.Entry(self.Size_selection_frame)
        self.size_desenho.grid(row=0, column=1, pady=5)
        
        # Botões
        ttk.Button(self.controls_frame, text="Confirm Class", command=self.GoToDrawing).grid(row=4, column=0, pady=5)
    ...
    def GoToDrawing(self):
        ...
        if(classe == ""):
            ...
            size_drawing = self.size_desenho.get()
            if(size_drawing == ""):
                size_drawing = 28
                self.window.withdraw()
                JanelaDesenho(self.window, self.path, classe, self.image, size_drawing)
            elif(size_drawing.isdigit() == False):
                messagebox.showerror("Error", "Enter a size!")
            else:
                size_drawing = int(size_drawing)
                self.window.withdraw()
                JanelaDesenho(self.window, self.path, classe, self.image, size_drawing)
...
class JanelaDesenho:
    def __init__(self, Config_window, path, DrawingName, base_image, size_drawing,count=0):
        ...
        self.size_drawing = size_drawing
    ...
    def save_canvas(self):
        ...
        ImagesSmall = ImgNormalizer(cv2.cvtColor(self.image2, cv2.COLOR_BGR2GRAY),self.size_drawing)
